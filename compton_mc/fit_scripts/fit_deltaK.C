...
void fit_deltaK()
{
	...
	sprintf( hname_tagh, "DeltaK/deltaK_tagh_pe" );
	sprintf( hname_tagm, "DeltaK/deltaK_tagm_pe" );
	...
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) 
	{
		...
		TH2F  *h2  = new TH2F( Form("h2_tagh_%d",tagh_counter), "#DeltaK vs. TAGH Counter", 
		...
		TF1 *f_fit = new TF1( Form("f_fit_tagh_%d",tagh_counter), "gaus", -1., 1. );
		
		f_fit->SetParameters( h1->GetMaximum(), h1->GetBinCenter(h1->GetMaximumBin()), 0.25 );
	...
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) 
	{
		...
		TH2F  *h2  = new TH2F( Form("h2_tagm_%d",tagm_counter), "#DeltaK vs. TAGM Counter", 
		...
		TF1 *f_fit = new TF1( Form("f_fit_tagm_%d",tagm_counter), "gaus", -1., 1. );
