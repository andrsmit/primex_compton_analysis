// $Id$
//
//    File: JEventProcessor_compton_analysis3.cc
// Created: Mon Nov 23 16:24:15 EST 2020
// Creator: andrsmit (on Linux ifarm1901.jlab.org 3.10.0-1062.4.1.el7.x86_64 x86_64)
//

#include "JEventProcessor_compton_analysis3.h"


extern "C"{
void InitPlugin(JApplication *app){
	InitJANAPlugin(app);
	app->AddProcessor(new JEventProcessor_compton_analysis3());
}
} // "C"
...
// Constructor
//------------------
JEventProcessor_compton_analysis3::JEventProcessor_compton_analysis3()
{
	// Set defaults:
	RUN_GROUP = 0;
	gPARMS->SetDefaultParameter( "COMPTON_ANALYSIS3:RUN_GROUP", RUN_GROUP );
...
// init
//------------------
jerror_t JEventProcessor_compton_analysis3::init(void)
{
	
	
	TDirectory *dir_compton_analysis3 = new TDirectoryFile( "compton_analysis3", "compton_analysis3" );
	dir_compton_analysis3->cd();
...
// brun
//------------------
jerror_t JEventProcessor_compton_analysis3::brun(JEventLoop *eventLoop, int32_t runnumber)
{
	...
	if( dgeom ){
		...
		dgeom->GetCCALPosition( m_ccalX, m_ccalY, m_ccalZ );
    	  	...
    	  	cerr << "No geometry accessbile to compton_analysis3 plugin." << endl;
...
// evnt
//------------------
jerror_t JEventProcessor_compton_analysis3::evnt(JEventLoop *eventLoop, uint64_t eventnumber)
{
...
// erun
//------------------
jerror_t JEventProcessor_compton_analysis3::erun(void)
{
...
// fini
//------------------
jerror_t JEventProcessor_compton_analysis3::fini(void)
{
...
// read in cut values
//------------------
void JEventProcessor_compton_analysis3::read_cuts()
{
...
//--------------------------------------------
int JEventProcessor_compton_analysis3::ccalLayer(int row, int col) {
...
//--------------------------------------------
int JEventProcessor_compton_analysis3::fcalLayer(int row, int col) {
...
// Fill Histograms
//--------------------------------------------
void JEventProcessor_compton_analysis3::fill_histograms( vector< ComptonCandidate_t > Comp_Candidates ) {
