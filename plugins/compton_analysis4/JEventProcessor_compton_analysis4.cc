// $Id$
//
//    File: JEventProcessor_compton_analysis4.cc
// Created: Mon Nov 23 16:24:15 EST 2020
// Creator: andrsmit (on Linux ifarm1901.jlab.org 3.10.0-1062.4.1.el7.x86_64 x86_64)
//

#include "JEventProcessor_compton_analysis4.h"


extern "C"{
void InitPlugin(JApplication *app){
	InitJANAPlugin(app);
	app->AddProcessor(new JEventProcessor_compton_analysis4());
}
} // "C"
...
// Constructor
//------------------
JEventProcessor_compton_analysis4::JEventProcessor_compton_analysis4()
{
	// Set defaults:
	RUN_GROUP = 0;
	gPARMS->SetDefaultParameter( "COMPTON_ANALYSIS4:RUN_GROUP", RUN_GROUP );
...
// init
//------------------
jerror_t JEventProcessor_compton_analysis4::init(void)
{
	
	
	TDirectory *dir_compton_analysis4 = new TDirectoryFile( "compton_analysis4", "compton_analysis4" );
	dir_compton_analysis4->cd();
...
// brun
//------------------
jerror_t JEventProcessor_compton_analysis4::brun(JEventLoop *eventLoop, int32_t runnumber)
{
	...
	if( dgeom ){
		...
		dgeom->GetCCALPosition( m_ccalX, m_ccalY, m_ccalZ );
    	  	...
    	  	cerr << "No geometry accessbile to compton_analysis4 plugin." << endl;
...
// evnt
//------------------
jerror_t JEventProcessor_compton_analysis4::evnt(JEventLoop *eventLoop, uint64_t eventnumber)
{
	...
	for( vector< const DFCALShower* >::const_iterator show1 = fcal_candidates.begin(); 
		...
		for( vector< const DCCALShower* >::const_iterator show2 = ccal_candidates.begin(); 
			...
			for( vector< const DBeamPhoton* >::const_iterator gam = beam_photons.begin();
				...
				if( fabs(brfdt) < 6.012 )
					bunch_val = 1;
				else if( (  -(6.012 + 4.*4.008) <= brfdt && brfdt <= -(6.012 + 3.*4.008) )
					|| ( (6.012 + 3.*4.008) <= brfdt && brfdt <=  (6.012 + 4.*4.008) ) )
					bunch_val = 0;
				else 
					continue;
...
// erun
//------------------
jerror_t JEventProcessor_compton_analysis4::erun(void)
{
...
// fini
//------------------
jerror_t JEventProcessor_compton_analysis4::fini(void)
{
...
// read in cut values
//------------------
void JEventProcessor_compton_analysis4::read_cuts()
{
...
//--------------------------------------------
int JEventProcessor_compton_analysis4::ccalLayer(int row, int col) {
...
//--------------------------------------------
int JEventProcessor_compton_analysis4::fcalLayer(int row, int col) {
...
// Fill Histograms
//--------------------------------------------
void JEventProcessor_compton_analysis4::fill_histograms( vector< ComptonCandidate_t > Comp_Candidates ) {
