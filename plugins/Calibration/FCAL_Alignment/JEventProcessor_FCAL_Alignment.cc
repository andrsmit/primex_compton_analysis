...
/*
Alignment of the FCAL using low-energy Compton scattering.

We select events with:
	1. Two showers in the FCAL
	2. No showers in the CCAL
	3. At least one beam photon (<5 GeV)
	4. 130 < |phi_1 - phi_2| < 230
	5. -1 < e_beam - (e_1 + e_2) < 1

Save following data to root tree:
	1. position of each shower (position from DFCALShower_factory, 
		with no additional corrections applied)
	2. energy of each shower
	3. beam energy
	4. whether or not each shower can be matched to a DTOFPoint in time and space
*/
...
jerror_t JEventProcessor_FCAL_Alignment::evnt(JEventLoop *loop, uint64_t eventnumber)
{
	...
	for(vector<const DCCALShower*>::const_iterator show = ccal_showers.begin();
		...
		if(fabs(loc_rf_dt)<6.0) { n_ccal_showers++; }
	}
	if(n_ccal_showers>0 || n_fcal_showers!=2) {
		japp->RootFillUnLock(this);
		return NOERROR;
	}
	
	const DFCALShower *show1 = good_fcal_showers[0];
	const DFCALShower *show2 = good_fcal_showers[1];
	...
	DVector3 pos2 = show2->getPosition_log() - vertex;
	...
	for(vector< const DBeamPhoton* >::const_iterator gam = beam_photons.begin();
		...
		if(!fcal_fiducial_cut(pos1,vertex) && !fcal_fiducial_cut(pos2,vertex)) {
		...
		if(!fcal_fiducial_cut2(pos1,vertex) && !fcal_fiducial_cut2(pos2,vertex)) {
	...
	if(n_good_cands==1) {
		...
		int tof_match1 = check_TOF_match(pos1, rfTime, vertex, tof_points);
		int tof_match2 = check_TOF_match(pos2, rfTime, vertex, tof_points);
...
// erun
//------------------
jerror_t JEventProcessor_FCAL_Alignment::erun(void)
{
	return NOERROR;
}
...
int JEventProcessor_FCAL_Alignment::fcal_fiducial_cut2(DVector3 pos, DVector3 vertex)
{
	int fid_cut = 0;
...
int JEventProcessor_FCAL_Alignment::ccal_fiducial_cut(DVector3 pos, DVector3 vertex)
{
	int fid_cut = 0;
...
int JEventProcessor_FCAL_Alignment::check_TOF_match(DVector3 pos1, double rfTime, 
