// $Id$
//
//    File: JEventProcessor_FCAL_Alignment.cc
// Created: Fri Jun  7 10:39:06 AM EDT 2024
// Creator: andrsmit (on Linux ifarm180302.jlab.org 5.14.0-362.24.2.el9_3.x86_64 x86_64)
//

/************************************************************************
Alignment of the FCAL with the beam using low-energy Compton Scattering:

Event Selection:
 * Two showers in the FCAL
 * No showers in the BCAL or the CCAL
 * At least one beam photon below 5 GeV
 * Loose coplanarity: |DeltaPhi-180| < 50 deg
 * Loose elasticity: |DeltaE| < 1 GeV

Write the following information to root TTree:
	* Position of each shower (directly from DFCALShower_factory)
	* Energy of each shower
	* Beam Energy
	* Whether or not each FCAL shower is matched to a DTOFPoint

************************************************************************/

#include "JEventProcessor_FCAL_Alignment.h"
...
void InitPlugin(JApplication *app){
	InitJANAPlugin(app);
	app->AddProcessor(new JEventProcessor_FCAL_Alignment());
}
} // "C"

thread_local DTreeFillData JEventProcessor_FCAL_Alignment::dTreeFillData;

//------------------
// init
//------------------
jerror_t JEventProcessor_FCAL_Alignment::init(void)
{
	
	TDirectory *dir_FCAL_Alignment = new TDirectoryFile("FCAL_Alignment", 
		"FCAL_Alignment");
	dir_FCAL_Alignment->cd();
	...
	dTreeInterface = DTreeInterface::Create_DTreeInterface(
		"fcal_compton", "fcal_compton.root");
	...
	locTreeBranchRegister.Register_Single<Int_t>("tof_match1");
	...
	locTreeBranchRegister.Register_Single<Int_t>("tof_match2");
...
// brun
//------------------
jerror_t JEventProcessor_FCAL_Alignment::brun(JEventLoop *eventLoop, int32_t runnumber)
{
...
// evnt
//------------------
jerror_t JEventProcessor_FCAL_Alignment::evnt(JEventLoop *loop, uint64_t eventnumber)
{
	//--------------------------------------------------------------------//
	// Check trigger:
	...
	for(vector<const DCCALShower*>::const_iterator show = locCCALShowers.begin();
		...
		h_ccal_rf_dt->Fill(rf_dt);
		
		if(fabs(rf_dt)<6.0) { locNCCALShowers++; }
	}
	...
	if(locNCCALShowers>0 || locNBCALShowers>0 || locNFCALShowers!=2) {
	...
	const DFCALShower *show2 = locGoodFCALShowers[1];
	...
	DVector3 pos2 = show2->getPosition_log() - locVertex;
	...
	for(vector< const DBeamPhoton* >::const_iterator gam = locBeamPhotons.begin();
		...
		if(!fcal_fiducial_cut(pos1,locVertex,1.0) && !fcal_fiducial_cut(pos2,locVertex,1.0)) {
		...
		if(!fcal_fiducial_cut(pos1,locVertex,2.0) && !fcal_fiducial_cut(pos2,locVertex,2.0)) {
			if(fabs(deltaPhi-180) < 50.) {
				if(e1 > 0.5 && e2 > 0.5 && fabs(deltaE) < 1.0) {
					cut_vals[4] = 1;
				}
	...
	if(locNGoodCands==1) {
		...
		dTreeFillData.Fill_Single<Double_t>("x2", show2->getPosition_log().X());
		dTreeFillData.Fill_Single<Double_t>("y2", show2->getPosition_log().Y());
		dTreeFillData.Fill_Single<Double_t>("z2", show2->getPosition_log().Z());
		dTreeFillData.Fill_Single<Double_t>("t2", show2->getTime());
		
		//-----------------------------------------------------------------//
		// Check for matches between FCAL showers and TOF:
		
		int tof_match1 = 0;
		double tof_dx1, tof_dy1, tof_dt1;
		check_TOF_match(pos1, locRFTime, locVertex, locTOFPoints, tof_dx1, tof_dy1, tof_dt1, 1.0);
		if(sqrt(pow(tof_dx1,2.0) + pow(tof_dy1,2.0)) < 8.0) {
			tof_match1 = 1;
		}
		
		int tof_match2 = 0;
		double tof_dx2, tof_dy2, tof_dt2;
		check_TOF_match(pos2, locRFTime, locVertex, locTOFPoints, tof_dx2, tof_dy2, tof_dt2, 1.0);
		if(sqrt(pow(tof_dx2,2.0) + pow(tof_dy2,2.0)) < 8.0) {
			tof_match2 = 1;
		}
		
		dTreeFillData.Fill_Single<Int_t>("tof_match1", tof_match1);
		dTreeFillData.Fill_Single<Int_t>("tof_match2", tof_match2);
...
// fini
//------------------
jerror_t JEventProcessor_FCAL_Alignment::fini(void)
{
...
int JEventProcessor_FCAL_Alignment::fcal_fiducial_cut(DVector3 pos, DVector3 vertex, 
	double cut_layers) {
...
void JEventProcessor_FCAL_Alignment::check_TOF_match(DVector3 pos1, double rfTime, 
