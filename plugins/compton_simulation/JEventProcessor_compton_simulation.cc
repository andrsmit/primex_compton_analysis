...
JEventProcessor_compton_simulation::JEventProcessor_compton_simulation()
{
	...
	f_deltaE_mu = new TF1(   "f_deltaE_mu", "pol3",                      BEAM_min_energy_cut, 12.0 );
	f_deltaE_mu->SetParameters( deltaE_mu_p0, deltaE_mu_p1, deltaE_mu_p2, deltaE_mu_p3 );
	
	f_deltaE_sig = new TF1( "f_deltaE_sig", "[0] + [1]/sqrt(x) + [2]/x", BEAM_min_energy_cut, 12.0 );
	f_deltaE_sig->SetParameters( deltaE_sig_p0, deltaE_sig_p1, deltaE_sig_p2 );
	
	
	
	f_deltaPhi_mu = new TF1(   "f_deltaPhi_mu", "pol3", BEAM_min_energy_cut, 12.0 );
	f_deltaPhi_mu->SetParameters(   deltaPhi_mu_p0,  deltaPhi_mu_p1,  deltaPhi_mu_p2,  deltaPhi_mu_p3 );
	
	f_deltaPhi_sig = new TF1( "f_deltaPhi_sig", "pol3", BEAM_min_energy_cut, 12.0 );
	f_deltaPhi_sig->SetParameters( deltaPhi_sig_p0, deltaPhi_sig_p1, deltaPhi_sig_p2, deltaPhi_sig_p3 );
	
	
	
	f_deltaK_mu = new TF1(   "f_deltaK_mu", "pol3", BEAM_min_energy_cut, 12.0 );
	f_deltaK_mu->SetParameters(   deltaK_mu_p0,  deltaK_mu_p1,  deltaK_mu_p2,  deltaK_mu_p3 );
	
	f_deltaK_sig = new TF1( "f_deltaK_sig", "pol3", BEAM_min_energy_cut, 12.0 );
...
jerror_t JEventProcessor_compton_simulation::brun(JEventLoop *eventLoop, int32_t runnumber)
{
	...
	if( dgeom ){
		...
		dgeom->GetCCALPosition( m_ccalX, m_ccalY, m_ccalZ );
    	  	...
    	  	cerr << "No geometry accessbile to compton_simulation plugin." << endl;
...
//--------------------------------------------
int JEventProcessor_compton_simulation::ccalLayer(int row, int col) {
...
//--------------------------------------------
int JEventProcessor_compton_simulation::fcalLayer(int row, int col) {
...
// Fill Histograms
//--------------------------------------------
void JEventProcessor_compton_simulation::fill_histograms( vector< ComptonCandidate_t > Comp_Candidates ) {
