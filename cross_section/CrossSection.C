...
void CrossSection()
{
	...
	sprintf( mc_dir, "%s/tag_sim/recRootFiles", pathName );
	...
	for( int i=0; i < n_cs_files; i++ ) {
		...
		cout << "Energy: " << gen_enVec[i] << "; CS: " << gen_csVec[i] << endl;
	...
	TH2F *h2e_tagh = new TH2F( "h2_tagm",  "#DeltaK vs. Counter", 
		102, 0.5, 102.5,  2000, -4.0, 4.0 );
	
	TH2F *h2_tagm  = new TH2F( "h2e_tagh", "#DeltaK vs. Counter", 
		274, 0.5, 274.5,  2000, -4.0, 4.0 );
	...
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
		...
		double loc_yield, loc_yieldE, loc_chi2;
	...
	for( int ib = 0; ib < n_bins1; ib++ ) {
		...
		cs1[ib]        = yield1Vec[ib] / (flux1Vec[ib] * locAcc * ne * mb);
		cs1E[ib]       = sqrt( 
	...
	for( int ib = 0; ib < n_bins2; ib++ ) {
		...
		cs2[ib]        = yield2Vec[ib] / (flux2Vec[ib] * locAcc * ne * mb);
		cs2E[ib]       = sqrt( 
