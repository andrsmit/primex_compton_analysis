...
int get_neighbor_empty(int tag_sys, int counter, int sim_case);

int fit_yield_empty(int tag_sys, int counter, TH1F *h1, TH1F *h1e, 
	...
	if(gSystem->AccessPathName(fname)) {
		...
		//return 0;
		neighbor_counter = get_neighbor_empty(tag_sys, counter, 0);
	...
	h1e->SetLineColor(kYellow);
	h1e->SetFillColor(kYellow);
	h1e->SetLineWidth(2);
	...
	h1->GetXaxis()->SetRangeUser(min_fit_x, max_fit_x);
	h1e->GetXaxis()->SetRangeUser(min_fit_x,max_fit_x);
	...
	double n_empty_exp = h1e->Integral(h1e->FindBin(min_fit_x), h1e->FindBin(max_fit_x));
	...
	fit->Constrain(2, 0.75*empty_frac_exp, 1.25*empty_frac_exp);
	...
	if(tag_sys==0) {
		...
		tagh_empty_frac[counter-1]     = empty_frac;
		tagh_empty_fracE[counter-1]    = empty_fracE;
		tagh_empty_frac_exp[counter-1] = empty_frac_exp;
	} else {
		...
		tagm_empty_frac[counter-1]     = empty_frac;
		tagm_empty_fracE[counter-1]    = empty_fracE;
		tagm_empty_frac_exp[counter-1] = empty_frac_exp;
	}
	...
	if(loc_DRAW_FITS) {
		...
		h_empty_fit->SetLineColor(kGreen);
		h_empty_fit->SetMarkerColor(kGreen);
		...
		leg->AddEntry(h_empty_fit, "Empty Target Bkgd (fit)", "l");
		leg->SetBorderSize(0);
		...
		TH1F *FitResultEmpty  = (TH1F*)h1e->Clone(Form("FitResultEmpty_%d_%d", tag_sys, counter));
		...
		FitResultEmpty->SetLineColor(kYellow);
		FitResultEmpty->SetFillColor(kYellow);
