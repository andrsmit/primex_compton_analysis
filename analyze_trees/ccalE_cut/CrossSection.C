...
void CrossSection()
{
	...
	double ccal_en   = 6.0;
	int ccal_en_int  = (int)(ccal_en*100);
	int ccal_en_int2 = (int)(ccal_en*10);
	...
	sprintf( root_fname,              "%s/analyze_trees/ccalE_cut/rootFiles/Be.root", 
		pathName );
	sprintf( empty_target_root_fname, "%s/analyze_trees/ccalE_cut/rootFiles/Be_empty.root", 
		pathName );
	...
	sprintf( hname_tagh_sim, "compton_systematics_sim/ccalE/deltaK_tagh_%02dccalE", ccal_en_int2 );
	sprintf( hname_tagm_sim, "compton_systematics_sim/ccalE/deltaK_tagm_%02dccalE", ccal_en_int2 );
	...
	for( int tagh_counter=1; tagh_counter<274; tagh_counter++ ) {
		h1_tagh[tagh_counter-1]  = 
			(TH1F*)fFull->Get(Form("%02dfcalE/tagh_%03d_%02dfcalE",ccal_en_int,
				tagh_counter,ccal_en_int))->Clone(Form("h1_tagh_%03d",tagh_counter));
		h1e_tagh[tagh_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%02dfcalE/tagh_%03d_%02dfcalE",ccal_en_int,
				tagh_counter,ccal_en_int))->Clone(Form("h1e_tagh_%03d",tagh_counter));
	}
	for( int tagm_counter=1; tagm_counter<102; tagm_counter++ ) {
		h1_tagm[tagm_counter-1]  = 
			(TH1F*)fFull->Get(Form("%02dfcalE/tagm_%03d_%02dfcalE",ccal_en_int,
				tagm_counter,ccal_en_int))->Clone(Form("h1_tagm_%03d",tagm_counter));
		h1e_tagm[tagm_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%02dfcalE/tagm_%03d_%02dfcalE",ccal_en_int,
				tagm_counter,ccal_en_int))->Clone(Form("h1e_tagm_%03d",tagm_counter));
	}
	...
	ofstream outf_tagh( Form("tagh_ccalE_%02d.txt", ccal_en_int2) );
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
	...
	ofstream outf_tagm( Form("tagm_ccalE_%02d.txt", ccal_en_int2) );
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) {
