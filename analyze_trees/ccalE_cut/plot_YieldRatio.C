...
void plot_YieldRatio()
{
	...
	vector<int> energy_vec = {00, 05, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60};
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ifstream inf_tagh( Form("tagh_ccalE_%02d.txt",energy_vec[ie]) );
		...
		ifstream inf_tagm( Form("tagm_ccalE_%02d.txt",energy_vec[ie]) );
	...
	for( int ic = 1; ic < MAX_COUNTERS; ic++ ) 
	{
		...
		for( int ie = 0; ie < n_ens; ie++ ) {
			...
			energies[ie]    = 0.1 * (double)energy_vec[ie];
			zeros[ie]       = 0.;
		...
		TGraphErrors *gYieldRatio = new TGraphErrors( n_ens, energies, yield_ratio, 
			zeros, yield_ratioE );
		gYieldRatio->GetXaxis()->SetTitle( "CCAL Energy Cut [GeV]" );
		...
		TGraphErrors *gAccRatio = new TGraphErrors( n_ens, energies, acc_ratio, 
			zeros, acc_ratioE );
		gAccRatio->GetXaxis()->SetTitle( "CCAL Energy Cut [GeV]" );
		...
		gCS->GetXaxis()->SetTitle( "CCAL Energy Cut [GeV]" );
		...
		if( loc_counter==0 ) {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_ccalE_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagh_ccalE_crossSection.pdf(", "pdf" );
			} else {
				canvas1->Print( "tagm_ccalE_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagm_ccalE_crossSection.pdf(",  "pdf" );
			}
		} else {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_ccalE_yieldRatio.pdf",    "pdf" );
