...
void CrossSection()
{
	...
	sprintf( root_fname,              "%s/analyze_trees/deltaK_cut/rootFiles/Be.root", 
		pathName );
	sprintf( empty_target_root_fname, "%s/analyze_trees/deltaK_cut/rootFiles/Be_empty.root", 
		pathName );
	...
	sprintf( hname_tagh_sim, "compton_systematics_sim/DeltaK/deltaK_tagh_%03dsigK", sig_int );
	sprintf( hname_tagm_sim, "compton_systematics_sim/DeltaK/deltaK_tagm_%03dsigK", sig_int );
	...
	for( int tagh_counter=1; tagh_counter<274; tagh_counter++ ) {
		h1_tagh[tagh_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigK/tagh_%03d_%03dsigK",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1_tagh_%03d",tagh_counter));
		h1e_tagh[tagh_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigK/tagh_%03d_%03dsigK",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1e_tagh_%03d",tagh_counter));
	}
	for( int tagm_counter=1; tagm_counter<102; tagm_counter++ ) {
		h1_tagm[tagm_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigK/tagm_%03d_%03dsigK",sig_int,tagm_counter,
				sig_int))->Clone(Form("h1_tagm_%03d",tagm_counter));
		h1e_tagm[tagm_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigK/tagm_%03d_%03dsigK",sig_int,tagm_counter,
	...
	ofstream outf_tagh( Form("tagh_sigK_%03d.txt", sig_int) );
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
	...
	ofstream outf_tagm( Form("tagm_sigK_%03d.txt", sig_int) );
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) {
...
int fit_yield( int tag_sys, int counter, TH1F *h1, double &yield, double &yieldE, double &chi2 )
{
	...
	*/
	...
	double min_fit_x = h1->GetBinCenter(h1->FindFirstBinAbove()+1);
	...
	*/
	...
	TF1 *f_bkgd = new TF1( Form("f_bkgd_%d_%d",tag_sys,counter), "pol3", min_fit_x, max_fit_x );
	f_bkgd->SetParameters( f_ls_fit->GetParameter(2), f_ls_fit->GetParameter(3), 
		f_ls_fit->GetParameter(4), f_ls_fit->GetParameter(5) );
	f_bkgd->SetLineColor(kGreen);
