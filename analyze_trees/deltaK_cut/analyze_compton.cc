...
int main(int argc, char **argv) {
	...
	sprintf( rootFile_pathName, 
		"/work/halld/home/andrsmit/primex_compton_analysis/analyze_trees/deltaK_cut/rootFiles" );
...
void compton_analysis(int run) 
{
	...
	for( int ic = 0; ic < n_candidates; ic++ ) {
		...
		if( !p_cut || !e_cut ) continue;
		
		
		for( int isig = 0; isig < N_SIGS; isig++ ) {
			
			int k_cut = 0;
			if( fabs(deltaK[ic] - deltaK_mu) < (cut_sigmas[isig]*deltaK_sig) ) k_cut = 1;
...
void write_histograms(char *name) {
	...
	for( int isig = 0; isig < N_SIGS; isig++ ) {
		...
		dir_sigs[isig] = new TDirectoryFile( Form("%03dsigK",sig_int), 
			Form("%03dsigK",sig_int) );
		dir_sigs[isig]->cd();
...
void init_histograms() 
{
	...
	for( int isig = 0; isig < N_SIGS; isig++ ) {
		...
		for( int tagh_counter=1; tagh_counter<=N_TAGH_COUNTERS; tagh_counter++ ) {
			h_tagh[isig][tagh_counter-1] = new TH1F( Form("tagh_%03d_%03dsigK",
				tagh_counter, sig_int), 
				Form("TAGH Counter %d", tagh_counter), 2000, -4.0, 4.0 );
		}
		for( int tagm_counter=1; tagm_counter<=N_TAGM_COUNTERS; tagm_counter++ ) {
			h_tagm[isig][tagm_counter-1] = new TH1F( Form("tagm_%03d_%03dsigK",
