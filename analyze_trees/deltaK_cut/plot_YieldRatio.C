...
void plot_YieldRatio()
{
	...
	vector<int> sigma_vec = {30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 100};
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ifstream inf_tagh( Form("tagh_sigK_%03d.txt",sigma_vec[ie]) );
		...
		ifstream inf_tagm( Form("tagm_sigK_%03d.txt",sigma_vec[ie]) );
	...
	for( int ic = 1; ic < MAX_COUNTERS; ic++ ) 
	{
		...
		TGraphErrors *gYieldRatio = new TGraphErrors( n_ens, sigmas, yield_ratio, 
			zeros, yield_ratioE );
		gYieldRatio->GetXaxis()->SetTitle( "#DeltaK Cut Width [#sigma]" );
		...
		TGraphErrors *gAccRatio = new TGraphErrors( n_ens, sigmas, acc_ratio, 
			zeros, acc_ratioE );
		gAccRatio->GetXaxis()->SetTitle( "#DeltaK Cut Width [#sigma]" );
		...
		gCS->GetXaxis()->SetTitle( "#DeltaK Cut Width [#sigma]" );
		...
		if( loc_counter==0 ) {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_deltaK_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagh_deltaK_crossSection.pdf(", "pdf" );
			} else {
				canvas1->Print( "tagm_deltaK_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagm_deltaK_crossSection.pdf(",  "pdf" );
			}
		} else {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_deltaK_yieldRatio.pdf",    "pdf" );
