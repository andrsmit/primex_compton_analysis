...
void compare_deltaK(int loc_cut_index=0)
{
	...
	if(true) {
		...
		comp_mc_dir = Form("%s/analyze_trees/phase1/analyze_mc/rootFiles/Run061321/compton/unsmeared", loc_pathname);
	...
	int tag_sys     = 0;
	int min_tag_counter = 50;
	int max_tag_counter = 60;
	...
	for(int ipar=0; ipar<4; ipar++) {
		...
		loc_sig += m_deltaE_sigma_pars[ipar]*pow(loc_eb, (double)ipar);
	}
	...
	vector<double> cut_sigs = {3.0, 5.0, 7.0, 10.0, 20.0};
	for(int i=0; i<(int)cut_sigs.size(); i++) {
		
		double loc_x1 = loc_mu + loc_sig*cut_sigs[i];
		double loc_x2 = loc_mu - loc_sig*cut_sigs[i];
		...
		double loc_fraction_data    = h_sub->Integral(h_sub->FindBin(loc_x2), h_sub->FindBin(loc_x1)) 
			/ n_compton_data;
		double loc_fraction_compton = h_compton->Integral(h_compton->FindBin(loc_x2), h_compton->FindBin(loc_x1)) 
			/ n_compton_sim;
		char buf[256];
		cout << "" << endl;
...
int get_pair_mc_hists(TString hist_fname, TString flux_fname) {
	...
	h2_tagh_pair = new TH2F("h2_tagh_pair", "#DeltaK; TAGH Counter; E_{1} + E_{2} - E_{#gamma} [GeV]",
		274, 0.5, 274.5, 1000, -8.0, 8.0);
	h2_tagm_pair = new TH2F("h2_tagm_pair", "#DeltaK; TAGH Counter; E_{1} + E_{2} - E_{#gamma} [GeV]",
...
int get_compton_mc_hists() {
	...
	h2_tagh_compton = new TH2F("h2_tagh_compton", "#DeltaK; TAGH Counter; E_{1} + E_{2} - E_{#gamma} [GeV]",
		274, 0.5, 274.5, 1000, -8.0, 8.0);
	h2_tagm_compton = new TH2F("h2_tagm_compton", "#DeltaK; TAGH Counter; E_{1} + E_{2} - E_{#gamma} [GeV]",
		102, 0.5, 102.5, 1000, -8.0, 8.0);
