...
void CrossSection_fcalE(double loc_cut=0.5)
{
	//==============================================================================//
	...
	root_fname              = Form("%s/analyze_trees/phase1/analyze_data/rootFiles/systematics/test_mgg_cut/%s_%03dnA_%s.root", 
		loc_pathname, TARGET_STR.Data(), BEAM_CURRENT, FIELD_STR.Data());
	
	empty_target_root_fname = Form("%s/analyze_trees/phase1/analyze_data/rootFiles/systematics/test_mgg_cut/%s_empty_%s.root", 
		loc_pathname, TARGET_STR.Data(), FIELD_STR.Data());
	
	int loc_cut_int = 1.e2 * loc_cut;
	
	hname_tagh = Form("fcalE/deltaK_tagh_%02dfcalE", loc_cut_int);
	hname_tagm = Form("fcalE/deltaK_tagm_%02dfcalE", loc_cut_int);
	...
	// Compton MC:
	comp_mc_dir     = Form("%s/analyze_trees/phase1/analyze_mc/rootFiles/Run%06d/compton/%03dnA_systematics/test_mgg_cut", 
		loc_pathname, mc_runNumber, BEAM_CURRENT);
	hname_tagh_comp = hname_tagh;
	hname_tagm_comp = hname_tagm;
	
	// e+e- Pair Production MC:
	pair_mc_dir     = Form("%s/analyze_trees/phase1/analyze_mc/rootFiles/Run061321/pair/systematics/test_mgg_cut", loc_pathname);
	...
	char out_fname[256];
	sprintf(out_fname, "fcalE/%s_%03dnA/tagh_cross_section_%02dfcalE.txt", TARGET_STR.Data(), BEAM_CURRENT, loc_cut_int);
	...
	sprintf(out_fname, "fcalE/%s_%03dnA/tagm_cross_section_%02dfcalE.txt", TARGET_STR.Data(), BEAM_CURRENT, loc_cut_int);
