...
void CrossSection()
{
	...
	sprintf( root_fname,              "%s/analyze_trees/deltaPhi_cut/rootFiles/Be.root", 
		pathName );
	sprintf( empty_target_root_fname, "%s/analyze_trees/deltaPhi_cut/rootFiles/Be_empty.root", 
		pathName );
	...
	sprintf( hname_tagh_sim, "compton_systematics_sim/DeltaPhi/deltaK_tagh_%03dsigPhi", sig_int );
	sprintf( hname_tagm_sim, "compton_systematics_sim/DeltaPhi/deltaK_tagm_%03dsigPhi", sig_int );
	...
	for( int tagh_counter=1; tagh_counter<274; tagh_counter++ ) {
		h1_tagh[tagh_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigPhi/tagh_%03d_%03dsigPhi",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1_tagh_%03d",tagh_counter));
		h1e_tagh[tagh_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigPhi/tagh_%03d_%03dsigPhi",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1e_tagh_%03d",tagh_counter));
	}
	for( int tagm_counter=1; tagm_counter<102; tagm_counter++ ) {
		h1_tagm[tagm_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigPhi/tagm_%03d_%03dsigPhi",sig_int,tagm_counter,
				sig_int))->Clone(Form("h1_tagm_%03d",tagm_counter));
		h1e_tagm[tagm_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigPhi/tagm_%03d_%03dsigPhi",sig_int,tagm_counter,
	...
	ofstream outf_tagh( Form("tagh_sigPhi_%03d.txt", sig_int) );
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
	...
	ofstream outf_tagm( Form("tagm_sigPhi_%03d.txt", sig_int) );
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) {
