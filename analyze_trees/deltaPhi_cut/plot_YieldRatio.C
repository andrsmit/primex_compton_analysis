...
void plot_YieldRatio()
{
	...
	vector<int> sigma_vec = {10, 20, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 100};
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ifstream inf_tagh( Form("tagh_sigPhi_%03d.txt",sigma_vec[ie]) );
		...
		ifstream inf_tagm( Form("tagm_sigPhi_%03d.txt",sigma_vec[ie]) );
	...
	for( int ic = 1; ic < MAX_COUNTERS; ic++ ) 
	{
		...
		if( loc_counter==0 ) {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_deltaPhi_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagh_deltaPhi_crossSection.pdf(", "pdf" );
			} else {
				canvas1->Print( "tagm_deltaPhi_yieldRatio.pdf(",   "pdf" );
				canvas2->Print( "tagm_deltaPhi_crossSection.pdf(",  "pdf" );
			}
		} else {
			if( tag_sys==0 ) {
				canvas1->Print( "tagh_deltaPhi_yieldRatio.pdf",    "pdf" );
