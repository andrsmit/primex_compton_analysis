...
void CrossSection()
{
	...
	sprintf( root_fname,              "%s/analyze_trees/rootFiles/Be.root",       pathName );
	sprintf( empty_target_root_fname, "%s/analyze_trees/rootFiles/Be_empty.root", pathName );
	...
	for( int tagh_counter=1; tagh_counter<274; tagh_counter++ ) {
		h1_tagh[tagh_counter-1]  = 
			(TH1F*)fFull->Get(Form("TAGH/tagh_%03d",tagh_counter))->Clone(
				Form("h1_tagh_%03d",tagh_counter));
		h1e_tagh[tagh_counter-1] = 
			(TH1F*)fEmpty->Get(Form("TAGH/tagh_%03d",tagh_counter))->Clone( 
				Form("h1e_tagh_%03d",tagh_counter));
	}
	for( int tagm_counter=1; tagm_counter<102; tagm_counter++ ) {
		h1_tagm[tagm_counter-1]  = 
			(TH1F*)fFull->Get(Form("TAGM/tagm_%03d",tagm_counter))->Clone(
				Form("h1_tagm_%03d",tagm_counter));
		h1e_tagm[tagm_counter-1] = 
			(TH1F*)fEmpty->Get(Form("TAGM/tagm_%03d",tagm_counter))->Clone( 
				Form("h1e_tagm_%03d",tagm_counter));
	}
