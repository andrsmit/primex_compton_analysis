...
void plot_YieldRatio()
{
	...
	vector<int> sigma_vec = {10, 20, 30, 35, 40, 45, 50, 55, 60, 65, 70, 80, 90, 100};
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ifstream inf_tagh( Form("txtfiles/tagh_sigE_%03d.txt",sigma_vec[ie]) );
		...
		ifstream inf_tagm( Form("txtfiles/tagm_sigE_%03d.txt",sigma_vec[ie]) );
