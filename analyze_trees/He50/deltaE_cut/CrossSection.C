...
void CrossSection()
{
	...
	sprintf( root_fname, 
		"%s/analyze_trees/He50/deltaE_cut/rootFiles/He_50.root",    pathName );
	sprintf( empty_target_root_fname, 
		"%s/analyze_trees/He50/deltaE_cut/rootFiles/He_empty.root", pathName );
	
	// photon flux file names:
	
	sprintf( tagh_flux_fname,              "%s/photon_flux/He_50_tagh_flux.txt",    pathName );
	sprintf( tagm_flux_fname,              "%s/photon_flux/He_50_tagm_flux.txt",    pathName );
	sprintf( empty_target_tagh_flux_fname, "%s/photon_flux/He_empty_tagh_flux.txt", pathName );
	sprintf( empty_target_tagm_flux_fname, "%s/photon_flux/He_empty_tagm_flux.txt", pathName );
	...
	sprintf( hname_tagh_sim, "compton_systematics_sim/DeltaE/deltaK_tagh_%03dsigE", sig_int );
	sprintf( hname_tagm_sim, "compton_systematics_sim/DeltaE/deltaK_tagm_%03dsigE", sig_int );
	...
	double target_density = 0.1217; // g/cm^3
	
	mb  = 1.e-27;
	ne  = target_density * (1./4.0026) * (6.02214e+23) * 2.0 * 29.5;
	...
	for( int tagh_counter=1; tagh_counter<274; tagh_counter++ ) {
		h1_tagh[tagh_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigE/tagh_%03d_%03dsigE",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1_tagh_%03d",tagh_counter));
		h1e_tagh[tagh_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigE/tagh_%03d_%03dsigE",sig_int,tagh_counter,
				sig_int))->Clone(Form("h1e_tagh_%03d",tagh_counter));
	}
	for( int tagm_counter=1; tagm_counter<102; tagm_counter++ ) {
		h1_tagm[tagm_counter-1]  = 
			(TH1F*)fFull->Get(Form("%03dsigE/tagm_%03d_%03dsigE",sig_int,tagm_counter,
				sig_int))->Clone(Form("h1_tagm_%03d",tagm_counter));
		h1e_tagm[tagm_counter-1] = 
			(TH1F*)fEmpty->Get(Form("%03dsigE/tagm_%03d_%03dsigE",sig_int,tagm_counter,
				sig_int))->Clone(Form("h1e_tagm_%03d",tagm_counter));
	}
	...
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
		...
		double loc_acc, loc_accE;
		double n_gen = 2.5e5;
		loc_acc  = f_acc->Eval(eb);
		loc_accE = sqrt( n_gen*loc_acc*(1.-loc_acc) ) / n_gen;
	...
	ofstream outf_tagh( Form("tagh_sigE_%03d.txt", sig_int) );
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
	...
	ofstream outf_tagm( Form("tagm_sigE_%03d.txt", sig_int) );
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) {
...
void get_acc()
{
	...
	// get the counter energies for a larger beam energy:
	
	double loc_tagh_en[274], loc_tagm_en[102];
	...
	for( int i=0; i<274; i++ ) {
		inf1 >> a >> b >> c;
		double deltaE  = loc_endpoint_energy - endpoint_energy_calib;
		double emin    = b * endpoint_energy_calib  +  deltaE;
		double emax    = c * endpoint_energy_calib  +  deltaE;
		loc_tagh_en[i] = 0.5 * (emin + emax);
	}
	inf1.close();
	...
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) {
		...
		char fname[256];
		sprintf( fname, "%s/tagh_%03d.root", mc_dir, tagh_counter );
		...
		double loc_eb = loc_tagh_en[tagh_counter-1];
		
		if( loc_eb > 8.9 && loc_eb < 8.92 ) continue;
		if( loc_eb > 8.72 && loc_eb < 8.73 ) continue;
		...
		TH2F *h2 = new TH2F( Form("h2_tagh_%d",tagh_counter), "DeltaK", 
			274, 0.5, 274.5, 2000, -4.0, 4.0 );
		...
		TH1F *h1 = (TH1F*)h2->ProjectionY( Form("h1_sim_tagh_%d",tagh_counter) );
		...
		TF1 *f_gaus = new TF1( Form("f_gaus_tagh_%d",tagh_counter), "gaus", -2., 2. );
		...
		TF1 *f_fit = new TF1( Form("f_fit_tagh_%d",tagh_counter), 
	...
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) {
		...
		char fname[256];
		sprintf( fname, "%s/tagm_%03d.root", mc_dir, tagm_counter );
		...
		double loc_eb = loc_tagm_en[tagm_counter-1];
		...
		TH1F *hbeam  = (TH1F*)fSim->Get( "compton_systematics_sim/beam" )->Clone(
			Form("h_beam_tagh_%d",tagm_counter) );
		double n_gen = 2.5e5 * (double)hbeam->GetMean();
		
		TH2F *h2 = new TH2F( Form("h2_tagm_%d",tagm_counter), "DeltaK", 
			102, 0.5, 102.5, 2000, -4.0, 4.0 );
		...
		TH1F *h1 = (TH1F*)h2->ProjectionY( Form("h1_sim_tagm_%d",tagm_counter) );
		if( h1->Integral() < 1.e1 ) continue;
		...
		TF1 *f_gaus = new TF1( Form("f_gaus_tagm_%d",tagm_counter), "gaus", -2., 2. );
		...
		TF1 *f_fit = new TF1( Form("f_fit_tagm_%d",tagm_counter), 
			double_gaus_fit, -2., 2., 10 );	
	...
	gAcc2->SetMarkerColor(kGreen);
	gAcc2->SetLineColor(kGreen);
	...
	f_acc = new TF1( "f_acc", "pol4", 6.0, 11.3 );
