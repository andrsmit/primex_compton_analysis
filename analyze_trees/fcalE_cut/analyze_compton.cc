...
void set_cuts() {
	...
	cut_energies[8]  =  0.40;
	cut_energies[9]  =  0.45;
	cut_energies[10] =  0.50;
	cut_energies[11] =  0.55;
	cut_energies[12] =  0.60;
	cut_energies[13] =  0.65;
	cut_energies[14] =  0.70;
	cut_energies[15] =  0.75;
	cut_energies[16] =  0.80;
	cut_energies[17] =  0.85;
	cut_energies[18] =  0.90;
	cut_energies[19] =  0.95;
...
int main(int argc, char **argv) {
	...
	sprintf( rootFile_pathName, 
		"/work/halld/home/andrsmit/primex_compton_analysis/analyze_trees/fcalE_cut/rootFiles" );
...
void compton_analysis(int run) 
{
	...
	for( int ic = 0; ic < n_candidates; ic++ ) {
		...
		for( int icut = 0; icut < N_CUTS; icut++ ) {
			...
			int fcal_e_cut = 0;
			if( fcal_e[ic] > cut_energies[icut] ) fcal_e_cut = 1;
