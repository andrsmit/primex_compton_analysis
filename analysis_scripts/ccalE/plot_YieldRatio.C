...
void plot_YieldRatio()
{
	...
	vector<int> ccal_ens = {00, 05, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60};
	
	int n_ens = (int)ccal_ens.size();
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ifstream inf_tagh( Form("tagh_ccalE_%02d.txt",ccal_ens[ie]) );
		...
		ifstream inf_tagm( Form("tagm_ccalE_%02d.txt",ccal_ens[ie]) );
	...
	double *ccal_energy  = new double[n_ens];
	...
	for( int ie = 0; ie < n_ens; ie++ ) {
		...
		ccal_energy[ie] = 0.1 * (double)ccal_ens[ie];
		zeros[ie]       = 0.;
	...
	TGraphErrors *gYieldRatio = new TGraphErrors( n_ens, ccal_energy, yield_ratio, 
		zeros, yield_ratioE );
	gYieldRatio->GetXaxis()->SetTitle( "Minimum CCAL Energy Cut [GeV]" );
	...
	TGraphErrors *gAccRatio = new TGraphErrors( n_ens, ccal_energy, acc_ratio, 
		zeros, acc_ratioE );
	gAccRatio->GetXaxis()->SetTitle( "Minimum CCAL Energy Cut [GeV]" );
	...
	TGraphErrors *gCS = new TGraphErrors( n_ens, ccal_energy, cs, zeros, csE );
	gCS->GetXaxis()->SetTitle( "Minimum CCAL Energy Cut [GeV]" );
