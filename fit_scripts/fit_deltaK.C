...
void fit_deltaK()
{
	...
	const bool DRAW_FITS_TAGM = true;
	...
	h2_tagh->Add( (TH2F*)fFull->Get("DeltaK/deltaK_tagh_ep") );
	h2_tagm->Add( (TH2F*)fFull->Get("DeltaK/deltaK_tagm_ep") );
	h2e_tagh->Add( (TH2F*)fEmpty->Get("DeltaK/deltaK_tagh_ep") );
	h2e_tagm->Add( (TH2F*)fEmpty->Get("DeltaK/deltaK_tagm_ep") );
	...
	for( int tagh_counter = 1; tagh_counter <= 274; tagh_counter++ ) 
	{
		...
		h1->GetXaxis()->SetTitle( "E_{Compton} - E_{#gamma} [GeV]" );
		...
		h1e->Rebin(rebins);
		h1e->GetXaxis()->SetTitle( "E_{Compton} - E_{#gamma} [GeV]" );
		...
		TF1 *f_fit = new TF1( Form("f_fit_tagh_%d",tagh_counter), 
			"gaus(0)+pol3(3)", -2.0, 2.0 );
		
		f_fit->SetParameters( h1->GetMaximum(), 
			h1->GetBinCenter(h1->GetMaximumBin()), 0.25 );
		...
		TF1 *f_draw = new TF1( Form("f_draw_tagh_%d",tagh_counter), "pol3", -3.0, 3.0 );
		f_draw->SetParameters( f_fit->GetParameter(3), f_fit->GetParameter(4), 
			f_fit->GetParameter(5), f_fit->GetParameter(6) );
		...
		canvas->Update();
		if( loc_counter==0 ) canvas->Print( "deltaK_tagh.pdf(", "pdf" );
		else                  canvas->Print( "deltaK_tagh.pdf",  "pdf" );
	...
	if( DRAW_FITS_TAGH ) canvas->Print( "deltaK_tagh.pdf)", "pdf" );
	
	
	
	//----------   Fit TAGM Counters   --------//
	
	vector<double>  en2Vec,  mu2Vec,  sig2Vec, chi22Vec;
	vector<double> en2EVec, mu2EVec, sig2EVec;
	...
	for( int tagm_counter = 1; tagm_counter <= 102; tagm_counter++ ) 
	{
		...
		TF1 *f_draw = new TF1( Form("f_draw_tagm_%d",tagm_counter), "pol3", -3.0, 3.0 );
		f_draw->SetParameters( f_fit->GetParameter(3), f_fit->GetParameter(4), 
			f_fit->GetParameter(5), f_fit->GetParameter(6) );
		f_draw->SetLineColor(kGreen);
		f_draw->SetLineStyle(2);
		...
		canvas->Update();
		if( loc_counter==0 ) canvas->Print( "deltaK_tagm.pdf(", "pdf" );
		else                  canvas->Print( "deltaK_tagm.pdf",  "pdf" );
	...
	if( DRAW_FITS_TAGM ) canvas->Print( "deltaK_tagm.pdf)", "pdf" );
	...
	*/
	...
	ofstream outfile1( "deltaK_tagh.dat" );
	for( int i=0; i<274; i++ ) {
	...
	ofstream outfile2( "deltaK_tagm.dat" );
	for( int i=0; i<102; i++ ) {
