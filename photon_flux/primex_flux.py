...
def main():
    ...
    htagh_flux      =  TH1F("TAGH Tagged Flux","TAGH Tagged Flux",274,0.5,274.5)
    htagh_flux_cor  =  TH1F("TAGH Tagged Flux Cor","TAGH Tagged Flux Cor",274,0.5,274.5)
    ...
    loc_deltaE = float(photon_endpoint[0][0]) - float(photon_endpoint_calib[0][0])
    ...
    for ii in range(274):
        ...
        tagh_energy_lo = float(photon_endpoint_calib[0][0])*float(tagh_scaled_energy[ii][1]) + loc_deltaE
        tagh_energy_hi = float(photon_endpoint_calib[0][0])*float(tagh_scaled_energy[ii][2]) + loc_deltaE
        tagh_energy = 0.5 * (tagh_energy_lo + tagh_energy_hi)
    ...
    for jj in range(102):
        ...
        tagm_energy_lo = float(photon_endpoint_calib[0][0])*float(tagm_scaled_energy[jj][1]) + loc_deltaE
        tagm_energy_hi = float(photon_endpoint_calib[0][0])*float(tagm_scaled_energy[jj][2]) + loc_deltaE
        tagm_energy = 0.5 * (tagm_energy_lo + tagm_energy_hi)
