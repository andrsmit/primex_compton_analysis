...
void getBeamAbsorption()
{
	...
	gPairCS->SetTitle( "Be Photon Attenuation Coefficient" );
	...
	TF1 *fPairCS = new TF1( "fPairCS", "pol3", 3.0, 12.0 );
	...
	gPairCS_high->SetTitle( "Be Photon Attenuation Coefficient" );
	...
	TF1 *fPairCS_high = new TF1( "fPairCS_high", "pol3", 3.0, 12.0 );
	gPairCS_high->Fit( "fPairCS_high", "R0Q" );
	
	TGraph *gPairCS_low = new TGraph( n_ens, photon_energy, pair_cs_low );
	gPairCS_low->SetTitle( "Be Photon Attenuation Coefficient" );
	...
	TF1 *fPairCS_low = new TF1( "fPairCS_low", "pol3", 3.0, 12.0 );
	...
	ofstream outf_tagh( "Be_tagh_fabs.dat" );
	for( int i=0; i<274; i++ ) {
	...
	ofstream outf_tagm( "Be_tagm_fabs.dat" );
	for( int i=0; i<102; i++ ) {
